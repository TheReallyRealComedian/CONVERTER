...
**Status:** ☑ done 2026-05-02 → see [inventory_2026-05.md](inventory_2026-05.md)
...
**Status:** ☑ done 2026-05-03 → branch `cleanup/stage-7-deps`. 27 → 25 production deps. Removed: `gevent` (unused; Dockerfile uses uvicorn worker), `mermaid` (frontend renders via CDN — and the PyPI package has broken metadata, uninstallable via modern pip). Comment-only kept: `gunicorn`, `uvicorn`, `opencv-python-headless` (transitive cv2 ABI pin). Full report: [dependency_audit_2026-05.md](dependency_audit_2026-05.md). Tests: 36/36 green, 5.2s.
