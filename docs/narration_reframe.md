...
> **Sequenz-Hinweis (✅ erledigt in NARR-5, `cbbcc88`)**: Der alte chatty-Flow **nutzte** `calculate_tag_guidance`/`format_dialogue_with_llm` bis zu seiner Stilllegung. Darum **baute NARR-1 den Treue-Pfad neu, ohne diese Funktionen aufzurufen** — **gelöscht** wurden sie mit der Alt-Flow-Abschaltung in **NARR-5**. Jeder Sprint blieb grün, der alte Flow lief bis zur bewussten Abschaltung weiter.
...
| **NARR-3** ✅ | M | **Token-Endpoint + async** (**☑ done 2026-06-30** — P1 `31a1768` · P2 `d8fd53c` · P3 `c18f4af`): `POST /api/narrations` (`NARRATION_TOKEN`, Spiegel `_authorize_card_write`, CSRF-exempt, fail-closed 503/401), `validate_turns`→400, `pending`-Conversion (flush→id→metadata→commit), `tasks.generate_narration_task` enqueue + job_id-Writeback. **Option B**: Worker **DB-frei** (rendert → `narration_<id>.wav` → returnt), **Web rekonziliert** (`reconcile_narration`, file-first) `pending`→`ready`+Dauer / `failed`+error beim Poll (`GET /api/narrations/<id>`) **und** beim Serve (vor dem ready-Gate). Auth-Matrix + Enqueue-Capture + Reconcile-Matrix; pytest **610→636** (+26). **Kein** Schema/Dep; einziger neuer Knopf `NARRATION_TOKEN` (env, fail-closed). |
| **NARR-4** ✅ | S/M | **Claude-Skill + mcp-Brief** (☑ 2026-06-30, Master-Doku, kein App-Code): [docs/narration_skill.md](narration_skill.md) (`erklaerbaer-narration` — Turn-Kontrakt, Treue-Doktrin „treu, fürs Hören geglättet", Voice-Katalog, 1–2 Speaker, → Delivery-Doktrin) + [docs/converter_mcp_narration_brief.md](converter_mcp_narration_brief.md) (3 Endpoints wrappen + `NARRATION_TOKEN`). **mcp-Wrap = Koordinator.** |
| **NARR-5** ✅ | L | **Library-Player + Alt-Flow stilllegen** (☑ done 2026-06-30 — P1 `aeb1877` · P2 `cbbcc88` · P3 dieser Commit): **P1 (additiv)** `audio_narration`-Player in `library_detail` (pollt `/api/narrations/<id>`: ready→`<audio>`+Dauer/Stimmen, pending→Spinner, failed→Fehler+Retry) + **`POST /api/narrations/<id>/retry`** (Session, CSRF, owner/type-404, **failed-only**→409, re-enqueue aus gespeicherten Render-Inputs; NARR-5 persistiert `mode`/`style_prompt`/`language_code` in `metadata` mit, Pre-NARR-5-Rows fallen graziös zurück). **P2 (subtraktiv, caller-first, grep-clean)** ganze Alt-Podcast-Generierung raus: UI-Tab/Pane aus `audio_converter` (~700 Z. geteiltes JS), alle 8 Routes + `app_pkg/podcasts.py`, `generate_podcast_task`, genai-Synth (`gemini/tts.py`+`synthesis.py`), Hack-Funktionen (`gemini/{script,prompts,dialogue}.py` — `calculate_tag_guidance`/`_TAG_DENSITY_MAP`/`format_dialogue_with_llm`/`parse_dialogue`), GeminiService-Facade-Methoden; `_GEMINI_VOICES`→`gemini/voices.py`, `GeminiService` **dormant** (stehen gelassen). **Behalten** (grep-bestätigt geteilt): `gemini/audio.py`+`client.py`, `GoogleTTSService`, `OUTPUT_DIR`, Narration, Audio-Transkription. Tests: `test_podcasts`(30)+`test_narration_synth`(4)+`test_full_flow.py` raus, pytest **646→612**. Live-Smoke (Player dark+light alle States + `/audio-converter`). |

Abhängigkeit: 1 ✅ → **1B ✅** → **2 ✅** → **3 ✅** → **4 ✅** → **5 ✅**. **NARRATION-Cluster komplett (2026-06-30).** Der Reframe ist durch: treuer Cloud-TTS-Renderer (1B), Library-Persistenz (2), Token-Endpoint + DB-freier async Worker + file-first Reconcile (3), Claude-Skill `erklaerbaer-narration` (4), Library-Player + Retry + komplette Stilllegung des alten chatty-Podcast-Flows (5). Real-Welt-Schritt für End-to-End-Audio (einmalig, falls noch offen): **`NARRATION_TOKEN` setzen** (Mac `.env` + Mintbox `.env` + converter-mcp-Config) + Mintbox-Deploy (`git pull` + `docker compose up -d --build`).
