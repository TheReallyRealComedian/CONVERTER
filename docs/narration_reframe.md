...
Abhängigkeit: 1 ✅ → **1B ✅** → **2 ✅** → **3 ✅** → (4, 5). **NARR-3 ✅ (2026-06-30) verbindet Renderer + Persistenz live — POST → `pending` → Worker rendert DB-frei → Web rekonziliert auf `ready`+Audio.** Real-Welt-Schritt vor dem ersten echten End-to-End-Audio: **`NARRATION_TOKEN` setzen** (Mac `.env` + Mintbox `.env` + converter-mcp-Config) + Mintbox-Deploy (`git pull` + `docker compose up -d --build` — zieht NARR-1Bs `google-cloud-texttospeech`-Dep mit). **NARR-4 (Claude-Skill `erklaerbaer-narration`) ist der nächste Sprint.**
