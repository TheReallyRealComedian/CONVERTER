...
- **F-1 strukturell abgeschlossen 2026-05-03.** Alle 14 Patterns + 3 Bug-Tickets implementiert. 6 Implementierungs-Cluster (A, B, C, D, E, Polish-1, Polish-2). Höchster Schweregrad-Befund (F1/F2 Empty-Submit silent, Sev 4) gefixt durch Pattern 1 in Cluster B. Alle Findings, deren Bug-Bestandteil F-006 für `document_converter` adressiert war, sind durch Cluster D's Backend-Whitelist mitgelöst. Pytest 37/37 grün durchgängig.
- **Hot-Fix 2026-05-03** (Production-Regression aus Cluster D): `templates/document_converter.html:29` enthielt `{{ ','.join('.' + ext for ext in accepted_extensions) }}` — Jinja2 unterstützt **keine** Generator-Expressions/List-Comprehensions. GET `/document-converter` lieferte konsequent 500 (TemplateSyntaxError, Compile-Time). Fix: `accept`-String im Route-Handler ([app_pkg/documents.py:30](app_pkg/documents.py#L30)) precomputen, Template konsumiert nur den fertigen String. Live-verifiziert per `app.test_request_context()`-Render im laufenden Container. Pytest hat den Bug nicht gefangen — Test-Suite rendert Templates nicht (genau Lesson #4 aus F-1: SDK-Mock-Boundary fängt UI/Template-Bugs nicht). **F-2 Feature-Wahl ausstehend.**
